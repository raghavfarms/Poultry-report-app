...
router.get('/default-date', requireFirmAccess, async (req, res) => {
  await loadFirm(req.query.firmId);
  res.json({ date: await earliestMissingDate(req.query.firmId) });
});

router.get('/opening', requireFirmAccess, async (req, res) => {
...
router.get('/service-status', requireFirmAccess, async (req, res) => {
  assertDate(req.query.date);
  await loadFirm(req.query.firmId);
...
router.get('/report', requireFirmAccess, async (req, res) => {
  const firm = await loadFirm(req.query.firmId);
...
router.post('/service-reset/:firmId/:assetId', requireFirmAccess, async (req, res) => {
...
router.put('/:firmId/:date', requireFirmAccess, async (req, res) => {
  ...
  if (req.user.role !== 'admin') {
    ...
    if (!existing) {
      ...
      if (req.params.date > nextDate) {
        return res.status(409).json({ message: `Please complete ${nextDate} first.` });
      }
    }
  }
