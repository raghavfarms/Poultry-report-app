...
export default function PurchaseOrderPage() {
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      {/* 1. Header & Actions */}
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-2.5 bg-white p-3.5 sm:p-4 rounded-xl border border-slate-200 shadow-xs">
        <div>
          <h1 className="text-lg sm:text-xl font-bold text-slate-800">Purchase Orders (PO)</h1>
          <p className="text-[11px] sm:text-xs text-slate-500">
            Create and track supplier orders, fulfillment, and deliveries
          </p>
        </div>
        <button
          onClick={handleOpenCreateModal}
          className="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs sm:text-sm px-3.5 py-2 rounded-lg shadow-xs transition flex items-center justify-center gap-1.5"
        >
          <span className="text-base leading-none">+</span> Create PO
        </button>
      </div>

      {/* Notifications */}
      {successMsg && (
        <div className="p-2.5 sm:p-3 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-lg text-xs font-medium">
          ✓ {successMsg}
        </div>
      )}
      {error && !isCreateModalOpen && (
        <div className="p-2.5 sm:p-3 bg-rose-50 border border-rose-200 text-rose-800 rounded-lg text-xs font-medium">
          ⚠ {error}
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
        {loading ? (
          ...
          <div className="p-6 text-center text-slate-500 text-xs">
            No purchase orders found. Click <strong>+ Create PO</strong> to get started.
          </div>
        ) : (
          <>
                  ...
                  {orders.map((po) => {
                    ...
                    return (
                      <tr key={po._id} className="hover:bg-slate-50/75 transition">
                        <td className="py-2.5 px-3 font-bold text-slate-900">{po.poNumber}</td>
                        ...
                        <td className="py-2.5 px-3 text-center">
                          <div className="flex items-center justify-center gap-1">
                            <button
                              ...
                              className="px-2 py-1 text-[11px] font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded transition"
                            >
                              View
                            </button>
                            {po.status === 'DRAFT' && (
                              <button
                                ...
                                className="px-2 py-1 text-[11px] font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded transition"
            ...
            <div className="sm:hidden divide-y divide-slate-100">
              {orders.map((po) => {
                ...
                return (
                  <div key={po._id} className="p-3 space-y-2">
      ...
      {/* 4. CREATE PO MODAL (Compact & Mobile-Optimized) */}
      {isCreateModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
          <div className="bg-white rounded-xl sm:rounded-2xl max-w-2xl w-full max-h-[92vh] flex flex-col shadow-2xl border border-slate-100 my-auto">
            {/* Modal Header */}
            <div className="flex justify-between items-center px-4 py-3 border-b border-slate-100 shrink-0">
              <h2 className="text-sm sm:text-base font-bold text-slate-800">Create Purchase Order</h2>
              <button
            ...
            <form onSubmit={handleCreatePo} className="flex flex-col flex-1 overflow-hidden">
              <div className="p-3.5 sm:p-4 overflow-y-auto space-y-3 flex-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                  ...
                  <div className="sm:col-span-1">
                    <label className="block text-[10px] font-bold text-slate-600 uppercase mb-0.5">Supplier *</label>
                    <select
                      value={formSupplier}
                  ...
                  {/* Order Date */}
                  <div>
                    <label className="block text-[10px] font-bold text-slate-600 uppercase mb-0.5">Order Date *</label>
                    <input
                      type="date"
                      value={formOrderDate}
                  ...
                  {/* Expected Delivery */}
                  <div>
                    <label className="block text-[10px] font-bold text-slate-600 uppercase mb-0.5">Expected Delivery</label>
                    <input
                      type="date"
                ...
                <div className="border border-slate-200 rounded-xl p-2.5 sm:p-3 bg-slate-50/60 space-y-2">
                  <div className="flex justify-between items-center px-0.5">
                    ...
                    <button
                      ...
                      className="text-xs font-bold text-emerald-700 hover:text-emerald-800 bg-emerald-50 hover:bg-emerald-100 px-2 py-0.5 rounded transition flex items-center gap-1"
                    >
                      + Add Item
                    </button>
                  </div>
                  ...
                  {formItems.map((item, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="bg-white p-2 sm:p-2.5 rounded-lg border border-slate-200 shadow-2xs space-y-1.5 sm:space-y-0 sm:flex sm:items-center sm:gap-2"
                      >
                        {/* Medicine Dropdown */}
                        <div className="flex-1 min-w-0 flex items-center gap-1">
                          <span className="text-[10px] font-bold text-slate-400 w-3.5 sm:hidden">{idx + 1}.</span>
                          <select
                            value={item.medicine}
                            onChange={(e) => handleItemChange(idx, 'medicine', e.target.value)}
                            required
                            className="w-full px-2 py-1.5 border border-slate-300 rounded-md text-xs font-medium focus:ring-1 focus:ring-emerald-500 focus:outline-none truncate"
                          >
                          ...
                          {formItems.length > 1 && (
                            <button
                              ...
                              className="sm:hidden text-slate-400 hover:text-rose-600 p-1 text-xs font-bold"
                              title="Remove"
                            >
                              ✕
                            </button>
                        ...
                        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                          ...
                          <div className="flex items-center w-24 sm:w-28 relative">
                            <input
                              ...
                              required
                              className="w-full pl-2 pr-8 py-1.5 border border-slate-300 rounded-md text-xs font-bold text-slate-800 focus:ring-1 focus:ring-emerald-500 focus:outline-none"
                            />
                            <span className="absolute right-1.5 text-[9px] text-slate-400 font-bold uppercase pointer-events-none truncate max-w-[28px]">
                          ...
                          <div className="w-20 sm:w-24">
                            <input
                              ...
                              className="w-full px-2 py-1.5 border border-slate-300 rounded-md text-xs text-right font-medium focus:ring-1 focus:ring-emerald-500 focus:outline-none"
                            />
                          </div>

                          {/* Subtotal */}
                          <div className="w-20 sm:w-24 text-right font-bold text-xs text-slate-800 truncate">
                          ...
                          {formItems.length > 1 && (
                            <button
                              type="button"
                              onClick={() => handleRemoveItem(idx)}
                              className="hidden sm:inline-flex text-slate-400 hover:text-rose-600 p-1 text-sm font-bold transition"
                              title="Remove item"
                            >
                              ✕
                            </button>
                  ...
                  {/* Grand Total bar */}
                  <div className="flex justify-between items-center pt-1.5 px-2.5 text-xs font-bold text-slate-700 bg-white rounded-lg p-2 border border-slate-200/80">
                    <span>Total ({formItems.length} {formItems.length === 1 ? 'item' : 'items'}):</span>
              ...
              <div className="flex justify-end gap-2 px-4 py-2.5 border-t border-slate-100 bg-slate-50/50 shrink-0">
                <button
                  type="button"
                  onClick={() => setIsCreateModalOpen(false)}
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-4 py-1.5 text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg shadow-xs transition disabled:opacity-50"
                >
      ...
      {/* 5. VIEW PO DETAILS MODAL */}
      {selectedPo && (
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
          <div className="bg-white rounded-xl sm:rounded-2xl max-w-xl w-full max-h-[92vh] flex flex-col shadow-2xl border border-slate-100 my-auto">
            {/* Header */}
            <div className="flex justify-between items-start px-4 py-3 border-b border-slate-100 shrink-0">
              <div>
                <div className="flex items-center gap-2">
                  <h2 className="text-sm sm:text-base font-bold text-slate-900">{selectedPo.poNumber}</h2>
                  <span
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">Order Date: {selectedPo.orderDate}</p>
              </div>
              <button
            ...
            <div className="p-3.5 sm:p-4 overflow-y-auto space-y-3 flex-1">
              ...
              <div className="grid grid-cols-2 gap-2 p-2.5 bg-slate-50 rounded-lg text-xs">
                ...
                <div>
                  ...
                  <span className="font-medium text-slate-700">{selectedPo.supplier?.contactPerson || '—'}</span>
              ...
              <div className="border border-slate-200 rounded-lg overflow-hidden">
                <table className="w-full text-left text-xs">
                  <thead className="bg-slate-50 font-bold text-slate-500 text-[10px] uppercase border-b border-slate-200">
                    <tr>
                      ...
                      <th className="py-2 px-2 text-center">Recv</th>
                      <th className="py-2 px-2 text-center">Bal</th>
                    ...
                    {selectedPo.items?.map((item, idx) => (
                      <tr key={idx}>
                        <td className="py-1.5 px-2.5">
                          ...
                          <div className="text-[10px] text-slate-400">{item.medicine?.code}</div>
                        </td>
                        <td className="py-1.5 px-2 text-center font-semibold text-[11px]">
                          {item.orderedQuantity} {item.medicine?.unit}
                        </td>
                        <td className="py-1.5 px-2 text-center text-purple-700 font-bold text-[11px]">
                        ...
                        <td className="py-1.5 px-2.5 text-right font-bold text-slate-800 text-[11px]">
                ...
                <div className="bg-slate-50 p-2.5 flex justify-between items-center text-xs font-bold border-t border-slate-200">
            ...
            {/* Footer */}
            <div className="flex justify-between items-center px-4 py-2.5 border-t border-slate-100 bg-slate-50/50 shrink-0">
              ...
              <button
                onClick={() => setSelectedPo(null)}
                className="ml-auto px-3.5 py-1.5 text-xs font-semibold text-slate-600 hover:bg-slate-100 rounded-lg transition"
              >
                Close
              </button>
            </div>
