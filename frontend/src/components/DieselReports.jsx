...
export default function DieselReports({ compact = false }) {
  ...
  const reportControls = <div className="no-print flex flex-wrap items-end gap-2"><label className="grid gap-1 text-xs font-bold text-slate-500">Firm<select className={`${inputClass} min-w-36`} value={firmFilter} onChange={(e) => setFirmFilter(e.target.value)}><option value="all">All accessible firms</option>{firms.map((firm) => <option key={firm._id} value={firm._id}>{firm.name}</option>)}</select></label><label className="grid gap-1 text-xs font-bold text-slate-500">From<input className={`${inputClass} cursor-pointer`} type="date" value={from} onClick={openDatePicker} onChange={(e) => setFrom(e.target.value)} /></label><label className="grid gap-1 text-xs font-bold text-slate-500">To<input className={`${inputClass} cursor-pointer`} type="date" value={to} onClick={openDatePicker} onChange={(e) => setTo(e.target.value)} /></label><button onClick={exportCsv} className={secondaryButton}>Export CSV</button><button onClick={() => window.print()} className={secondaryButton}>Print</button><button onClick={openNew} disabled={!firms.length} className={primaryButton}>＋ Add entry</button></div>;
