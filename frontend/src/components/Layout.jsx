...
function Sidebar({ open, close }) {
  ...
  return <>
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-slate-200 bg-white p-4 transition-transform lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <nav className="report-scroll flex-1 space-y-1 overflow-y-auto pr-1" onClick={close}>
        ...
        {modules.map(([slug, label, icon]) => <NavLink key={slug} to={`/reports/${slug}`} className={linkClass}><span className="w-5 text-center">{icon}</span>{label}</NavLink>)}
...
export default function Layout() {
  ...
  return <div className="min-h-screen bg-[#f4f7f4]"><Sidebar open={open} close={() => setOpen(false)} /><header className="sticky top-0 z-20 flex h-12 items-center border-b border-slate-200 bg-white/95 px-4 backdrop-blur lg:ml-72 lg:px-5"><button onClick={() => setOpen(true)} className="mr-3 rounded-lg border border-slate-200 px-2 py-1.5 lg:hidden">☰</button><span className="text-sm font-semibold text-slate-600">Daily farm reporting</span></header><main className="p-3 sm:p-4 lg:ml-72 lg:p-5"><Outlet /></main></div>;
}
